...
- **勾选（7 项）**：9.1 / 9.2 / 9.5 / 9.7 / 9.8 / 9.9 / 9.11 —— 9.11 的两项内容（Windows 运行时前置条件与自备途径、全离线变体的构建方式与约 140MB 体积代价）由计划① 写入，本轮核对通过，故一并勾选。
- **保留未勾（4 项，需真机/人工）**：9.3 断网冒烟、9.4 四平台构建（本轮仅实测 macOS arm64）、9.6 跨 WebView、9.10 Windows 前置条件 —— 已做的替代证据见本记录与账本 R101／计划① T20 的「需人工确认项」。
- 提交：`ba3cde8`。tasks.md 用 blob 精确定位入暂存（仅本轮的 7 行勾选），**未混入并行工作流第 6/7/8 组未提交的勾选**（工作区仍保留其 28 行）；账本追加 R102。
