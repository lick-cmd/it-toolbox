...
- [ ] 7.1 实现 `core/web/url-codec.ts`：组件/整体 URI/表单三种模式的编码与解码，含不完整转义序列检测
- [ ] 7.2 实现 `core/web/json-diff.ts`：基于 JSON 路径的结构化 diff，区分新增/删除/修改并识别类型变化，忽略键序差异
- [ ] 7.3 实现 `core/web/jwt.ts`：三段拆分解码、时间声明可读化、过期状态判定、段数与载荷异常处理
- [ ] 7.4 实现 `core/web/url-analyzer.ts`：协议/用户名/密码/主机/端口/Origin/路径/路径分段/查询参数（含重复键）/片段，默认端口识别与百分号解码
- [ ] 7.5 编写上述 Core 的 Vitest 用例，覆盖 spec 中全部 Scenario
- [ ] 7.6 实现 URL 编码/解码工具（三模式切换、非法转义提示）
- [ ] 7.7 实现 JSON 差异比较工具（双输入面板、交换按钮、变更类型着色、无差异态）
- [ ] 7.8 实现 JWT 解析器工具（三段分区展示、时间声明可读化、过期/有效期提示、「签名未校验」显式说明）
- [ ] 7.9 实现 URL 分析器工具（参数表格逐行展示同名参数、编码与解码形式并列、非绝对 URL 的补全建议）
...
- [ ] 9.1 核对 17 个工具在侧栏与搜索中均可发现，且分类归属正确
- [ ] 9.2 执行全量 Core 单测，确认 spec 中每个 Scenario 均有对应用例且全部通过
- [ ] 9.3 断网冒烟：离线状态下逐个使用 17 个工具，确认无功能降级且无出网请求
- [ ] 9.4 macOS（arm64 + x64）与 Windows（x64 + arm64）四平台构建，确认产物可安装可启动
- [ ] 9.5 校验单平台安装包体积不超过 15MB，超出则分析并裁剪依赖
- [ ] 9.6 跨 WebView 冒烟：在 macOS WKWebView 与 Windows WebView2 上验证剪贴板、文件拖放与 WebCrypto 行为一致
- [ ] 9.7 编写 README：开发启动、构建命令、新增工具的步骤说明（目录 + 两个文件 + Core 函数）
- [ ] 9.8 运行 `openspec validate it-toolbox-app` 与一致性校验测试，确认全部通过
- [ ] 9.9 离线四层验证：CSP 拒绝外发、开发期检测生效、产物扫描通过、远程图片与外部链接行为符合预期
- [ ] 9.10 Windows 前置条件验证：在具备 WebView2 的机器上零网络安装并启动；在缺失运行时的机器上确认失败提示清晰
- [ ] 9.11 README 补充：Windows 运行时前置条件与自备途径、全离线安装包变体的构建方式与体积代价
